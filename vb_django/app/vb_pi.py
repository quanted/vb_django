...
class CVPlusPI:
    ...
    @staticmethod
    def run(y_train: np.array, cv_yhat_train: np.array, cv_yhat_predict: np.array, alpha=0.05,
            collapse_reps: str = 'pre_mean', true_y_predict=None):
        ...
        if collapse_reps == 'drop':
            ...
            assert False, f'collapse_reps:{collapse_reps} not developed'
        if collapse_reps == 'post_mean':
            ...
            upper_q = np.mean(np.sort((cv_yhat_predict + cv_train_err[:, :, None]), axis=1)[:, alpha_idx - 1, :],
                              axis=0)
        else:
            q_n = train_n * n_reps
            alpha_idx = int(np.ceil((q_n + 1) * (1 - alpha)))
            lower_q = np.sort((cv_yhat_predict - cv_train_err[:, :, None]).reshape(n_reps * train_n, predict_n),
                              axis=0)[-alpha_idx, :]
            # dimensions:(n_reps,n_splits,train_n,predict_n)-> (., predict_n) -> (predict_n,)
            upper_q = np.sort((cv_yhat_predict + cv_train_err[:, :, None]).reshape(n_reps * train_n, predict_n),
                              axis=0)[alpha_idx - 1, :]
        if not true_y_predict is None:
            assert true_y_predict.shape[-1] == predict_n
            coverage = np.sum(np.ones_like(true_y_predict)[
                                  np.where((true_y_predict > lower_q) & (true_y_predict < upper_q))]) / predict_n
    ...
    @staticmethod
    def run_comparison(collapse_options=['None', 'pre_mean', 'pre_median', 'post_mean', 'drop'], n=50,
                       make_data_kwargs=dict(train_n=50, k=40, predict_n=1000, n_splits=5), return_results=False):
        ...
        for _ in range(n):
            ...
            for c_o in collapse_options:
                record[c_o].append(CVPlusPI.run(y_train, cv_yhat_train, cv_yhat_predict, true_y_predict=true_y_predict,
                                                collapse_reps=c_o))
        print('')  # for a new line
        print(*[(f'option: {c_o}', 'avg width: ', np.mean([l[1] - l[0] for l in r]), 'avg coverage: ',
                 np.mean([l[2] for l in r]), 'min coverage: ', min(l[2] for l in r)) for c_o, r in record.items()],
              sep='\n')
        if return_results:
            return record
